...
// مسار ملفات الإدارة KING/
const adminFolder = path.join(__dirname, 'KING');
...
function loadCommands(dir) {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      loadCommands(fullPath); // تحميل المجلدات الفرعية
      ...
      try {
        ...
        if (required.name && typeof required.execute === 'function') {
          ...
          console.log(`⚠️ Loaded admin/utility file: ${file}`);
        }
      } catch (err) {
...
// تحميل أوامر البوت من المجلدات الرئيسية
foldersToLoad.forEach(folder => loadCommands(folder));

// تحميل الملفات الإدارية من KING/
loadCommands(adminFolder);
