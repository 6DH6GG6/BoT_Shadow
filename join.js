...
module.exports = {
    ...
    async execute(chatId, args, message) {
        ...
        const OWNER_ID = parseInt(process.env.OWNER_ID); // ضع رقمك هنا أو في env
...
LINK = 〖${link}〗`;
        ...
        if (message.chat && (message.chat.type === "group" || message.chat.type === "supergroup" || message.chat.type === "channel")) {
            ...
            if (message.chat.type !== "channel") {
                try {
                    const res = await axios.get(`https://api.telegram.org/bot${TOKEN}/getChatAdministrators`, {
                        params: { chat_id: chatIdGroup }
                    });
